...
We do see a slight increasing trend of PHQ-9 scores over the three months in the
young adult age-group (20-39) compared to the other two age-groups.
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
# reading in model
rdat <- load(file = "../model_fit/fit2.rds")
```
...
We can use the `{tidybayes}` package to visualize our results. 
...
Kay M (2023). _tidybayes: Tidy Data and Geoms for Bayesian Models_. doi:
10.5281/zenodo.1308151 (URL: https://doi.org/10.5281/zenodo.1308151), R package
version 3.0.4, <URL: http://mjskay.github.io/tidybayes/>.
